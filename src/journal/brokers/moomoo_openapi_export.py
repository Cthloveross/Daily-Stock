...
OPENAPI_EXPORT_PARSER_VERSION = "1.0.0"
...
class OpenApiReconciliation:
    ...
    def as_dict(self) -> dict[str, Optional[int]]:
        return {
            ...
            for name in sorted(_RECONCILIATION_FIELDS)
        }
...
def _parse_order(
    ...
    *,
    ...
    _validate_fields(row, _ORDER_FIELDS, path)
...
def _parse_fill(
    ...
    *,
    ...
    if filled_at < parent.ordered_at:
        raise MoomooOpenApiExportError(f"{path}.create_time precedes the parent order")
    quantity = _decimal(
        row["qty"], f"{path}.qty", minimum=Decimal("0"), strictly_positive=True
    )
...
def _validate_summary(
    ...
    *,
    ...
    _validate_fields(summary, _SUMMARY_FIELDS, "summary")
    ...
    _validate_fields(counts, _SUMMARY_COUNT_FIELDS, "summary.counts")
    parsed_counts = {
    ...
    _validate_fields(
        deduplicated, _DEDUPLICATION_FIELDS, "summary.deduplicated_rows"
    )
    for name in _DEDUPLICATION_FIELDS:
    ...
    if environment == "LIVE":
        ...
        if parsed_reconciliation != reconciliation.as_dict():
            raise MoomooOpenApiExportError(
                "summary.reconciliation does not match parsed observations"
            )
        failures = {
            name: value
            for name, value in reconciliation.as_dict().items()
            if value not in (None, 0)
        }
        expected_status = "passed" if not failures else "failed"
        expected_ready = bool(filled_orders) and not failures
        expected_warnings = [
            f"{name}={value}" for name, value in sorted(failures.items())
        ]
    ...
    if (
        ...
        or reconciliation_status != expected_status
        or list(warnings) != expected_warnings
        or fee_batches != expected_fee_batches
    ):
        raise MoomooOpenApiExportError(
            "summary readiness facts do not match parsed observations"
        )
    return analysis_ready, reconciliation_status, warnings
...
def parse_openapi_export(payload: Mapping[str, Any]) -> OpenApiExportPreview:
    ...
    _validate_fields(account, _ACCOUNT_FIELDS, "account")
    ...
    _validate_fields(records, _RECORD_CONTAINER_FIELDS, "records")
    ...
    analysis_ready, reconciliation_status, warnings = _validate_summary(
        summary,
        account=account,
        window=window,
