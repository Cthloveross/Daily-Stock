...
#      ⚠ 「活下来」这个说法已被下面的 v8 更正推翻（分层按持仓时长定义、循环性
#      是构造性的；诚实记分下差异只剩 ~0.1pp）。0.5 现只是参考刻度，见 v8。
...
# v8: 两处**诚实性修正**，没有新增任何评分或信号：
#   1) K 线时间戳口径修正——Moomoo ``time_key`` 按 K 线**结束**时间打标，此前被
#      当成开始时间用，实际取到的常规时段是 09:25–15:55：混进一根盘前 K 线、
#      丢掉收盘集合竞价那根。修正后所有窗口/中位数/位移/形态读数都会变化
#      （数值口径变更，故升版本），详见 intraday_bursts 的 v8 口径说明。
#   2) 近 30 分位移的**循环性更正**——0.5 这条线来自按持仓时长分组的分层，
#      循环性是构造性的；改成只用 15 分钟检查点**向前**计分后，速度未死与
#      速度已死的 P(>0) 差异只剩 ~0.1pp。该列因此只描述过去 30 分钟，
#      不承载任何前向信息（文案随之去掉「活下来」的存活框架）。
INTRADAY_TOP_SIGNAL_VERSION = "intraday_session_evidence_v8"
