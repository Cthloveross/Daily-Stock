...
_CATEGORY_DEFINITIONS: List[Dict[str, Any]] = [
    ...
    },
    {
        "category": "phase0",
        "title": "Phase 0 (v4)",
        "description": "Daily-Stock v4 Mirror layer: Phase tracking, Journal inbox, Regime, Breakout, Lab, Options.",
        "display_order": 70,
    },
    {
...
_FIELD_DEFINITIONS: Dict[str, Dict[str, Any]] = {
    ...
    "JOURNAL_AI_MODEL": {
        "title": "Journal AI Primary Model",
        "description": "Optional primary model used only for position-episode AI reviews. When empty, reviews inherit the Agent primary model. Bare model names are normalized to openai/<model> unless they match a configured router alias.",
        "category": "ai_model",
        "data_type": "string",
        "ui_control": "text",
        "is_sensitive": False,
    ...
    "JOURNAL_AI_FALLBACK_MODELS": {
        "title": "Journal AI Fallback Models",
        "description": "Comma-separated fallbacks used only for position-episode AI reviews. If this setting is absent, an explicit Journal primary model falls back through the current Agent chain; saving an explicit empty value disables Journal fallback.",
        "category": "ai_model",
        "data_type": "string",
        "ui_control": "text",
        "is_sensitive": False,
    ...
    # See New-docs/ for full specification. These fields are registered
    # in Stage 0 but most features only activate in later stages.
    # ------------------------------------------------------------------
    "CURRENT_PHASE": {
        "title": "Current Phase",
        "description": "Daily-Stock v4 journey phase. 0=Mirror, 1=Lab activation, 2=Mixed strategy, 3=Core positions. Gates which Lab features are available.",
        "category": "phase0",
        "data_type": "integer",
        "ui_control": "select",
        "is_sensitive": False,
        ...
        "default_value": "0",
        "options": [
            {"label": "Phase 0 — Mirror", "value": "0"},
            {"label": "Phase 1 — Lab", "value": "1"},
            {"label": "Phase 2 — Mixed", "value": "2"},
            {"label": "Phase 3 — Core", "value": "3"},
        ],
        "validation": {"min": 0, "max": 3},
        "display_order": 10,
    },
    "PHASE_CONTEXT_INJECTION": {
        "title": "Phase-Aware Prompt Injection",
        "description": "When on, Agent skills automatically receive the current Phase and allowed skill set in their system prompt prefix.",
        "category": "phase0",
        "data_type": "boolean",
        "ui_control": "switch",
        "is_sensitive": False,
    ...
    "PERSONAL_TRADING_STYLE": {
        "title": "Personal Trading Style",
        "description": "Free-form statement of your trading style (stops, entry rules, held horizons). Injected into option_trader / leap_explorer / trend_follower skill prompts.",
        "category": "phase0",
        "data_type": "string",
        "ui_control": "textarea",
        "is_sensitive": False,
    ...
    # --- Journal ---
    "INBOX_DIR": {
        "title": "Journal Inbox Directory",
        "description": "Local folder watched for broker CSV exports (Moomoo etc.). Files landing here are auto-imported in Stage 11.",
        "category": "phase0",
        "data_type": "string",
        "ui_control": "text",
        "is_sensitive": False,
        "is_required": False,
        "is_editable": True,
        "default_value": "~/Daily-Stock-Inbox",
        "options": [],
        "validation": {},
        "display_order": 20,
    },
    "PROCESSED_DIR": {
        "title": "Journal Processed Directory",
        "description": "Where successfully imported CSV files are moved after processing.",
        "category": "phase0",
        "data_type": "string",
        "ui_control": "text",
        "is_sensitive": False,
        "is_required": False,
        "is_editable": True,
        "default_value": "~/Daily-Stock-Processed",
        "options": [],
        "validation": {},
        "display_order": 21,
    },
    "CSV_FORMAT": {
        "title": "Journal CSV Format",
        "description": "Broker CSV dialect used by the Journal parser. Extensible to ibkr / alpaca later.",
        "category": "phase0",
        "data_type": "string",
        "ui_control": "select",
        "is_sensitive": False,
        "is_required": False,
        "is_editable": True,
        "default_value": "moomoo_us",
        "options": [
            {"label": "Moomoo US", "value": "moomoo_us"},
        ],
        "validation": {"enum": ["moomoo_us"]},
        "display_order": 22,
    },
    # --- Regime ---
    "REGIME_ENABLED": {
        "title": "Regime Classifier",
        "description": "Master switch for the six-dimension daily Regime Score. Stage 3 populates it; Stage 4 adds the morning brief.",
        "category": "phase0",
        "data_type": "boolean",
        "ui_control": "switch",
        "is_sensitive": False,
    ...
    "REGIME_MIN_SCORE": {
        "title": "Regime Minimum Trading Score",
        "description": "Scores below this value route into `no_trade` action hint. Used by Breakout Q1 filter.",
        "category": "phase0",
        "data_type": "integer",
        "ui_control": "number",
        "is_sensitive": False,
    ...
    "REGIME_THRESHOLD_AGGRESSIVE": {
        "title": "Regime Aggressive Threshold",
        "description": "Score >= this becomes `aggressive`. Default 75.",
        "category": "phase0",
        "data_type": "integer",
        "ui_control": "number",
        "is_sensitive": False,
    ...
    "REGIME_THRESHOLD_STANDARD": {
        "title": "Regime Standard Threshold",
        "description": "Score >= this becomes `standard` (unless >= aggressive). Default 55.",
        "category": "phase0",
        "data_type": "integer",
        "ui_control": "number",
        "is_sensitive": False,
    ...
    "REGIME_THRESHOLD_CAUTIOUS": {
        "title": "Regime Cautious Threshold",
        "description": "Score >= this becomes `cautious` (unless >= standard). Below this is `no_trade`. Default 35.",
        "category": "phase0",
        "data_type": "integer",
        "ui_control": "number",
        "is_sensitive": False,
    ...
    "REGIME_BRIEF_ENABLED": {
        "title": "Regime Morning Brief",
        "description": "Send a daily Telegram brief summarising today's Regime Score. Activated in Stage 4.",
        "category": "phase0",
        "data_type": "boolean",
        "ui_control": "switch",
        "is_sensitive": False,
    ...
    "REGIME_BRIEF_TIME_ET": {
        "title": "Regime Brief Time (ET)",
        "description": "Wall-clock time in America/New_York for the morning brief. Two UTC crons (EDT/EST) guard by actual ET time.",
        "category": "phase0",
        "data_type": "time",
        "ui_control": "time",
        "is_sensitive": False,
    ...
    "REGIME_DB_VERSION": {
        "title": "Regime Model Version",
        "description": "Identifier for the Regime scoring model. Bumped when scorers change; stored alongside each score row for reproducibility.",
        "category": "phase0",
        "data_type": "string",
        "ui_control": "text",
        "is_sensitive": False,
    ...
    # --- Breakout ---
    "BREAKOUT_VOLUME_MIN": {
        "title": "Breakout Volume Minimum",
        "description": "Lower bound on breakout bar volume relative to 20-day average. Default 1.5.",
        "category": "phase0",
        "data_type": "number",
        "ui_control": "number",
        "is_sensitive": False,
    ...
    "BREAKOUT_VOLUME_MIN_MULTIPLE": {
        "title": "Breakout Volume Confirmation Multiple",
        "description": "Cumulative 5-minute volume vs same-window 20-day average threshold used by volume_check.",
        "category": "phase0",
        "data_type": "number",
        "ui_control": "number",
        "is_sensitive": False,
    ...
    "BREAKOUT_TIMEFRAMES": {
        "title": "Breakout Timeframes",
        "description": "Comma-separated timeframes checked by timeframe_check for multi-interval alignment.",
        "category": "phase0",
        "data_type": "array",
        "ui_control": "textarea",
        "is_sensitive": False,
        "is_required": False,
        "is_editable": True,
        "default_value": "2min,5min,15min,1day",
        "options": [],
        "validation": {"min_items": 1},
        "display_order": 42,
    },
    "BREAKOUT_RS_MIN": {
        "title": "Breakout Minimum Relative Strength",
        "description": "Minimum relative strength vs SPY (as percent) required to pass rs_check.",
        "category": "phase0",
        "data_type": "number",
        "ui_control": "number",
        "is_sensitive": False,
    ...
    "BREAKOUT_RETEST_WINDOW_MIN": {
        "title": "Breakout Retest Window (min)",
        "description": "Upper bound on minutes spent waiting for a breakout retest before declaring a failure. Used by filter decision tree.",
        "category": "phase0",
        "data_type": "integer",
        "ui_control": "number",
        "is_sensitive": False,
    ...
    "RETEST_WINDOW_MINUTES": {
        "title": "Retest Tracker Window (min)",
        "description": "How long the retest tracker observes price action after a breakout before classifying success/failure.",
        "category": "phase0",
        "data_type": "integer",
        "ui_control": "number",
        "is_sensitive": False,
    ...
    # --- Lab (Phase 1 activation, defined in Stage 0 for registry completeness) ---
    "LAB_LEAP_DELTA_MIN": {
        "title": "LEAP Delta Min",
        "description": "Lower bound of delta band used by LEAP Explorer candidate selection.",
        "category": "phase0",
        "data_type": "number",
        "ui_control": "number",
        "is_sensitive": False,
    ...
    "LAB_LEAP_DELTA_MAX": {
        "title": "LEAP Delta Max",
        "description": "Upper bound of delta band used by LEAP Explorer candidate selection.",
        "category": "phase0",
        "data_type": "number",
        "ui_control": "number",
        "is_sensitive": False,
    ...
    "SHADOW_TRADES_ENABLED": {
        "title": "Shadow Trades",
        "description": "Enable virtual-trade tracking. Activated in Phase 1 to practise directional judgement without account risk.",
        "category": "phase0",
        "data_type": "boolean",
        "ui_control": "switch",
        "is_sensitive": False,
    ...
    "DEFAULT_RISK_FREE_RATE": {
        ...
        "description": "Default annualised risk-free rate for Black-Scholes pricing and Greek computation. Review quarterly.",
        "category": "phase0",
        "data_type": "number",
        "ui_control": "number",
        "is_sensitive": False,
    ...
    "IV_SNAPSHOT_ENABLED": {
        "title": "IV Snapshot Cron",
        "description": "When on, a daily job stores the at-the-money IV for watchlist symbols for later IV-Rank calculations.",
        "category": "phase0",
        "data_type": "boolean",
        "ui_control": "switch",
        "is_sensitive": False,
