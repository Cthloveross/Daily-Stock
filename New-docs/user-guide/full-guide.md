...
> 完整说明见 [LLM 配置指南](../configuration/LLM_CONFIG_GUIDE.md)（三层配置、渠道模式、Vision、Agent、排错）。
...
| `LLM_CHANNELS` | 渠道名称列表（逗号分隔），配合 `LLM_{NAME}_*` 使用，详见 [LLM 配置指南](../configuration/LLM_CONFIG_GUIDE.md) | - | 否 |
...
| `OLLAMA_API_BASE` | Ollama 本地服务地址（如 `http://localhost:11434`），详见 [LLM 配置指南](../configuration/LLM_CONFIG_GUIDE.md) | - | 可选 |
...
更完整的图文排查请看 [New-docs/integrations/bots/feishu-bot-config.md](../integrations/bots/feishu-bot-config.md)。
### Telegram
...
详见 [LLM 配置指南](../configuration/LLM_CONFIG_GUIDE.md)。默认使用时你只需要理解主模型、备选模型和模型渠道；如果进入这一节，说明你要直接使用底层 [LiteLLM](https://github.com/BerriAI/litellm) 路由能力，无需单独启动 Proxy 服务。
...
**视觉模型（图片提取股票代码）**：详见 [LLM 配置指南 - Vision](../configuration/LLM_CONFIG_GUIDE.md#41-vision-模型图片识别股票代码)。
