...
> Quick start guide available in [README_EN.md](../user-guide/README_EN.md). This document covers advanced configuration.
...
> Full details: [LLM Config Guide](../configuration/LLM_CONFIG_GUIDE_EN.md) (three-tier config, channels, Vision, Agent, troubleshooting).
...
| `LLM_CHANNELS` | Channel names (comma-separated), use with `LLM_{NAME}_*`, see [LLM Config Guide](../configuration/LLM_CONFIG_GUIDE_EN.md) | - | No |
...
| `OLLAMA_API_BASE` | Ollama local service address (e.g. `http://localhost:11434`), see [LLM Config Guide](../configuration/LLM_CONFIG_GUIDE_EN.md) | - | Optional |
...
For a full illustrated troubleshooting guide, see [New-docs/integrations/bots/feishu-bot-config.md](../integrations/bots/feishu-bot-config.md).
...
See [LLM Config Guide](../configuration/LLM_CONFIG_GUIDE_EN.md). Most users only need to think in terms of primary models, fallback models, and channels; this section is for expert users who want direct access to the underlying [LiteLLM](https://github.com/BerriAI/litellm) routing capabilities. No separate Proxy service is required.
...
**Vision model (image stock code extraction)**: See [LLM Config Guide - Vision](../configuration/LLM_CONFIG_GUIDE_EN.md#41-vision-model-image-stock-code-extraction).
