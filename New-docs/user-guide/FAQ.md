...
> 完整说明见 [LLM 配置指南](../configuration/LLM_CONFIG_GUIDE.md)。
...
**避坑**：不要使用 `OPENAI_BASE_URL` 配置 Ollama，否则系统会错误拼接 URL（如 404、`api/generate/api/show`）。详见 [LLM 配置指南](../configuration/LLM_CONFIG_GUIDE.md) 示例 4 与渠道示例。
...
> 完整配置示例见 [LLM 配置指南 → 示例 4（Ollama）](../configuration/LLM_CONFIG_GUIDE.md#example-4-ollama)。
